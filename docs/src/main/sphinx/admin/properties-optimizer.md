...
At the default value the enumeration explores every join order within its budget, so
lowering this property is the only way to make the optimizer fall back on the greedy,
budget-limited search; raising it lets more joins be reordered together.
